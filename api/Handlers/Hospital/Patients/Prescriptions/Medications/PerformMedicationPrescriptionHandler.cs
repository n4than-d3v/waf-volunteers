...
public class PerformMedicationPrescription : IRequest<IResult>
{
    ...
    public PerformMedicationPrescription WithId(int id)
    {
        PrescriptionId = id;
        return this;
    }
...
public class PerformMedicationPrescriptionHandler : IRequestHandler<PerformMedicationPrescription, IResult>
{
    ...
    {
        var prescription = await _repository.Get<PatientPrescriptionMedication>(request.PrescriptionId, action: x => x.Include(y => y.Patient));
        ...
        prescription.Patient.LastUpdatedDetails = DateTime.UtcNow;
