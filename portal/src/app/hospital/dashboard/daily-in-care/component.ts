...
@Component({
  selector: 'hospital-dashboard-daily-in-care',
  standalone: true,
  templateUrl: './component.html',
  styleUrls: ['./component.scss'],
})
export class HospitalDashboardChartInCareComponent
  implements AfterViewInit, OnDestroy
{
  ...
  @ViewChild('inCareLine') inCareLine?: ElementRef<HTMLCanvasElement>;
  ...
  private tryBuildChart() {
    ...
    if (!this.inCareLine?.nativeElement) {
      setTimeout(() => this.tryBuildChart(), 0);
      return;
    }

    this.chart?.destroy();
    this.buildInCareLine();
  }
  ...
  private buildInCareLine() {
    const years = Object.keys(this.dashboard.patientsInCareByDate);

    const labels = this.buildYearDayLabels();

    const datasets: ChartDataset<'line'>[] = years.map((year, idx) => {
      const dateMap = this.dashboard.patientsInCareByDate[+year];
    ...
    this.chart = new Chart(this.inCareLine!.nativeElement, config);
  }
}
